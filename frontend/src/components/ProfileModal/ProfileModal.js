...
import { StyledCard, StyledTitle, HeaderWrapper, StyledSubtitle, StyledOption, StyledParagraph, ButtonWrapper } from './styles';
...
const ProfileModal = ({ user, forceExit, callback }) => {
    ...
    return <>
        <StyledCard exit={exit} onAnimationEnd={() => exit && callback && callback()}>
            <StyledImg src={closeIcon} clickable onClick={() => setExit(true)} />
            <HeaderWrapper>
                <StyledTitle>Premium plan is activated</StyledTitle>
                <StyledSubtitle>Available premium sessions left: <b>{user.premiumSessionsLeft || 0}</b></StyledSubtitle>
            </HeaderWrapper>
            <StyledOption onClick={() => setConfirmModal({
                ...
                <StyledParagraph><b>9.99PLN</b></StyledParagraph>
            </StyledOption>
            <StyledOption onClick={() => setConfirmModal({
            ...
            <ButtonWrapper>
                <Button secondary small onClick={() => setConfirmModal({
