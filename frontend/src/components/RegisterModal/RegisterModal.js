...
import { ButtonWrapper, StyledParagraph, RowWrapper, ColumnWrapper, PlanButton, PlanBox, PlanAmount, PlanDescription, PlanPrice } from './styles';
...
const RegisterModal = ({ input, callback }) => {
    const [data, setData] = useState({ email: '', password: '', repeatPassword: '', bundleCount: 1 });
    ...
    const getPrice = (amount) => {
        return (amount * bundlePrice - getSaved(amount, false))
            .toLocaleString('pl-PL', { style: 'currency', currency: 'PLN', currencyDisplay: 'code' });
    };

    const getSaved = (amount, formatted = true) => {
        const saved = amount * (amount - 1) * 0.5 * discount * bundlePrice;
        return formatted ? saved.toLocaleString('pl-PL', { style: 'currency', currency: 'PLN', currencyDisplay: 'code' }) : saved;
    };
        ...
        primaryButton: [
            {
                ...
                action: async () => await validateInput() && await register() && nextStep(),
            },
            {
                content: 'Continue',
                action: () => nextStep(),
            ...
            <>
                <ColumnWrapper>
                    <PlanButton
                        ...
                        onClick={() => setData(d => ({ ...d, bundleCount: Math.max(d.bundleCount - 1, 1) }))} />
                    <PlanBox>
                    ...
                    <PlanButton
                        ...
                        onClick={() => setData(d => ({ ...d, bundleCount: Math.min(d.bundleCount + 1, maxBundleCount) }))} />
                </ColumnWrapper>
                <StyledParagraph>
                    For every 10 more sessions purchased you will get an additional <em>10% discount</em>.
                    At once you can only acquire 100 sessions.
                </StyledParagraph>
            </>,
            <RowWrapper>
                <StyledParagraph>
                    ...
                    <br /><br />
                    You have chosen to purchase <em>{data.bundleCount * bundleAmount} sessions</em> at once for{' '}
                    <em>{getPrice(data.bundleCount)}</em>. That way you will save <em>{getSaved(data.bundleCount)}</em>.
                    <br /><br />
                    To continue please confirm your email address by opening a link we sent you.
                    <br />
    ...
    const register = async () => {
        ...
        const result = await fetch(`${process.env.REACT_APP_URL}/api/v1/users/create`, {
            method: 'POST',
            body: JSON.stringify({
                email: data.email,
