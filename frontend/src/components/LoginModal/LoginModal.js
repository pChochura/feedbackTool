...
const LoginModal = ({ input, callback }) => {
    ...
    const attemptLogin = async () => {
        ...
        let response = await fetch(`${process.env.REACT_APP_URL}/api/v1/users`, { method: 'POST' });

        const incomingDigest = ClientDigestAuth.analyze(response.headers.get('WWW-Authenticate'));
        const digest = ClientDigestAuth.generateProtectionAuth(incomingDigest, email, password, {
            method: 'POST',
            uri: '/api/v1/users',
            counter: 1,
        ...
        <>
            <Modal
                ...
                <StyledInput
                    ...
                    value={password} />
                <ButtonWrapper>
                    <Button onClick={() => attemptLogin()} loading={loading} disabled={loading}>Login</Button>
                </ButtonWrapper>
                <StyledParagraph>
                    Or{' '}
                    <b onClick={() => callback && callback({ createAccount: true, email, password })}>create an account</b>
                </StyledParagraph>
            </Modal>
